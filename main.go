...
func main() {
	...
	watcherService := services.NewWatcherService(conf, conf.Watchers, datasourceService, conf.Datasources, scheduler, elastic)
	go func() {
	...
	watcherController := controllers.NewWatcherController(watcherService, datasourceService)
